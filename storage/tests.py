...
class FileStorageAPITestCase(APITestCase):
    ...
    def test_file_upload(self):
        file = SimpleUploadedFile("testfile.txt", b"hello world", content_type="text/plain")
        response = self.client.post("/api/files/", {"file": file})
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(File.objects.count(), 1)

    def test_file_list(self):
        file = SimpleUploadedFile("list.txt", b"list content", content_type="text/plain")
    ...
    def test_file_download(self):
        test_file = SimpleUploadedFile("download.txt", b"download content", content_type="text/plain")
    ...
    def test_special_link_download(self):
        test_file = SimpleUploadedFile("link.txt", b"link content", content_type="text/plain")
        upload_response = self.client.post("/api/files/", {"file": test_file})
        self.assertEqual(upload_response.status_code, status.HTTP_201_CREATED)
        ...
        response = self.client.get(f"/api/files/shared/{file_obj.special_link}/")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertIn('Content-Disposition', response)
